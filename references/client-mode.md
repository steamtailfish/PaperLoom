...
仓库和完整包均包含 skill、代码、文档、示例，以及 `fonts/` 中的清单和五个原始字体文件，便于用户在获授权环境中直接使用。`scripts/package_skill.py --variant full` 会检查字体是否齐全。`--variant github` 生成的源码归档不含独立字体文件；仅在使用该归档或本地缺少字体时，才需按 `fonts/README.md` 从自己有权使用的来源补齐字体。
