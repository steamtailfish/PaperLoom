...
仓库和完整使用包按用户要求带入上述原始字体，以及本 README 和 manifest，供已有合法使用权的环境使用。需要公开发布带字体的包时，应先取得对应再分发授权。

若本地缺少字体，从自己获授权的 Windows/Office 安装或字体供应方取得字体，按上述文件名放入本目录。不要从不明网站自动下载字体。不同合法版本的 hash 可能不同，应核对 family、style 和许可，再更新自己的 manifest。
